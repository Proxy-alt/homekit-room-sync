...
- Resolving every accessory's Serial Number means one extra CLI call per accessory at generation time — run concurrently (12 at a time by default) rather than one at a time, since these are all independent, mostly-I/O-bound calls to the same running HomeClaw app. Verified live against a real ~150-accessory home: resolves in well under 10 seconds this way (vs. over a minute sequentially), with identical results. It's best-effort: if listing/resolving fails outright, generation falls back to name-only matching for everything rather than aborting.
- Serial-number matching works for anything HA's `homekit` integration created — including cameras, locks, and TVs/media players, which HAP requires to run as their own standalone accessory rather than folded into the shared multi-accessory bridge. Don't read that structural distinction (HomeKit's own `bridged: true/false` flag) as "connected to HA" vs. not — we initially did, and were wrong: verified live that a real lock reporting `bridged: false` still had its Serial Number set to its HA entity_id. It only falls back to name matching for accessories genuinely paired directly to Apple Home by their own manufacturer, entirely outside HA (e.g. a lock's own native pairing, if you have one set up alongside its HA integration) — always check the `--dry-run` output for anything reported as not found. [haconnect](https://github.com/canadianblaken/ha-homekit-bridge-connect) goes further by physically actuating each device and watching for the matching state change instead of trusting any identifier to already agree, but it's very new (no real version history yet), so it isn't the default here.
- Resolving Serial Numbers always does a *live* per-accessory read now, not the native CLI's `--no-refresh`/cached mode — despite that flag's own description calling it "ideal for serial number... sweeps", we verified live that it returns a stale placeholder for characteristics HomeClaw hadn't read live before, which silently produces false "no serial number" results. A live sweep of ~130 real accessories took under a minute.
- **HA "Group" helper entities** (e.g. a Light Group combining several individual lights into one, exposed to HomeKit as a single accessory) get their room from their members. A group entity has no device of its own to inherit an area from the normal way, so without this it'd be silently skipped and left wherever HomeKit happened to place it — verified live: a `light.basement_stairs` group accessory had its Serial Number correctly set (HA really was managing it), but was sitting in an unrelated default room because nothing upstream could tell it where it belonged. If every member of the group agrees on one Area, the group now inherits it too; if members span more than one Area, it's left unresolved rather than guessed.
